...
/**
 * نمک هش بازدیدکننده — هرگز در دیتابیس ذخیره نمی‌شود، یعنی حتی با در دست
 * داشتن فایل دیتابیس هم نمی‌شود هش را به آی‌پی برگرداند.
 *
 * ⚠️ روی سرور واقعی که با `cluster.js` چند پردازشی اجرا می‌شود، این مقدار
 * **باید بین همه‌ی پردازش‌ها یکی باشد**. اگر هر پردازش نمک خودش را بسازد،
 * هش یک بازدیدکننده در هر پردازش متفاوت درمی‌آید و یک نفر تا چهار بار
 * به‌عنوان «بازدیدکننده‌ی یکتا» شمرده می‌شود. برای همین `cluster.js` نمک
 * را قبل از fork می‌سازد و در `STATS_SALT` می‌گذارد.
 *
 * در حالت تک‌پردازشی (`node server.js`) متغیر خالی است و همین‌جا یکی
 * ساخته می‌شود.
 */
const SALT = process.env.STATS_SALT || crypto.randomBytes(32).toString('hex');
