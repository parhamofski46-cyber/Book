...
if (cluster.isPrimary) {
  ...
  /**
   * «نمکِ» شمارش بازدیدکننده‌ی یکتا هم دقیقاً به همین دلیل اینجا ساخته
   * می‌شود.
   *
   * اگر هر فرزند نمک خودش را بسازد، هش یک بازدیدکننده در هر پردازش
   * متفاوت درمی‌آید و یک نفر تا چهار بار به‌عنوان «بازدیدکننده‌ی یکتا»
   * شمرده می‌شود — یعنی عدد داشبورد تا چهار برابر باد می‌کند.
   *
   * عمداً در دیتابیس ذخیره نمی‌شود و فقط در حافظه می‌ماند: با در دست
   * داشتن فایل دیتابیس هم نمی‌شود هش را به آی‌پی برگرداند. هزینه‌اش این
   * است که با هر ری‌استارت عوض می‌شود و بازدیدکننده‌های همان روز یک‌بار
   * دوباره شمرده می‌شوند — که در برابر باگ چهاربرابری، ناچیز است.
   */
  process.env.STATS_SALT = require('crypto').randomBytes(32).toString('hex');
