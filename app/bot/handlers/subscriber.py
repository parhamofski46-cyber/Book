...
from app.payments.base import format_money, get_provider
...
async def on_successful_payment(message: Message, session: AsyncSession, locale: str) -> None:
    ...
    await grant_access(message.bot, session, payment=payment, locale=locale, actor="telegram")
...
async def claim_payment(call: CallbackQuery, session: AsyncSession, locale: str) -> None:
    ...
    await _notify_owner_of_claim(call, session, owner=owner, channel=channel, plan=plan)
    await call.answer()


async def _notify_owner_of_claim(
    call: CallbackQuery, session: AsyncSession, *, owner, channel, plan
) -> None:
    text = (
        f"💰 Payment claimed\n"
...
async def grant_access(
    ...
    if link is None:
        await bot.send_message(subscriber.telegram_user_id, t("error.generic", locale))
        return
